...
/// NoteItem 클래스: createdAt은 "yyyy-MM-dd HH:mm:ss" 형식의 문자열로 저장합니다.
class NoteItem {
  final String text;
  final String createdAt; // "yyyy-MM-dd HH:mm:ss" 형태의 문자열
...
class _NotePageState extends State<NotePage> {
  ...
  Future<void> _loadAllData() async {
    ...
    if (user != null) {
      // === 로그인된 사용자: Firebase에서 메모+알림을 모두 불러온다 ===
      ...
      // === 비로그인 사용자: SharedPreferences에서 불러온다 ===
      final prefs = await SharedPreferences.getInstance();

      // 1) 메모(JSON 문자열 리스트) 불러오기
